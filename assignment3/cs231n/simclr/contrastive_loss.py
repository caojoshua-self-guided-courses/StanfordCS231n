...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        def l(i, j):
            sims = torch.zeros(N*2)
            for k2 in range(N*2):
                sims[k2] = sim(out[i], out[k2])
            sims = torch.exp(sims / tau)
            sims[i] = 0
            return -torch.log(sims[j] / torch.sum(sims))
...
def sim_positive_pairs(out_left, out_right):
    ...
    dot = torch.sum(out_left * out_right, dim=1)
    pos_pairs = dot / (torch.linalg.norm(out_left, dim=1) * torch.linalg.norm(out_right, dim=1))
...
def compute_sim_matrix(out):
    ...
    sim_matrix /= norm
    sim_matrix = torch.rot90(torch.rot90(sim_matrix, dims=(0, 1)) / norm, dims=(1,0))
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cpu'):
    ...
    left_range = torch.arange(N)
    right_range = torch.arange(start = N, end = 2 * N)
    sim_pairs = torch.concat((sim_matrix[left_range, right_range], sim_matrix[right_range, left_range]))
    ...
    loss = torch.sum(-torch.log(numerator[0:N] / denom[0:N]) - torch.log(numerator[N:] / denom[N:])) / (2*N)
